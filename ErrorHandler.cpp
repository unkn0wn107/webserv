...
void ErrorHandler::handleError(const std::exception& ex) {
    ...
    // For example, you might want to log the error to a file or send a notification
    ...
    // However, for a server, it might be better to keep running unless it's a critical error
}
...
void ErrorHandler::handleError(const std::string& errorMessage) {
    ...
    // Similar to the exception handling, handle the error without crashing the server
...
void ErrorHandler::handleFatalError(const std::string& errorMessage) {
    ...
    // Exiting might be necessary if the error leaves the server in an unstable state
