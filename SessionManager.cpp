...
std::map<std::string, std::string> SessionManager::getSession(const std::string& sessionId) {
    ...
    if (sessions.find(sessionId) != sessions.end()) {
        ...
        return std::map<std::string, std::string>(); // Return empty map if session not found
    }
}

void SessionManager::setSession(const std::string& sessionId, const std::map<std::string, std::string>& sessionData) {
