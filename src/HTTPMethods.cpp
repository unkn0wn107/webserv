...
/*   Updated: 2024/06/20 16:04:52 by agaley           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
HTTPResponse* HTTPMethods::_handlePostRequest(HTTPRequest& request) {
  ...
  if (contentType == "") {
    ...
    std::string extension =
        HTTPResponse::getExtensionFromContentType(contentType);
    path += "." + extension;
  }
  ...
  if (file) {
    ...
    response->setBody("<html><body>File uploaded successfully. File path: " +
                      path + "</body></html>");
    return response;
  }
...
HTTPResponse* HTTPMethods::_handleDeleteRequest(HTTPRequest& request) {
  ...
  if (remove(path.c_str()) == 0) {
    ...
    response->setBody("<html><body>File deleted successfully. File path: " +
                      path + "</body></html>");
    return response;
  } else {
...
HTTPResponse* HTTPMethods::handleRequest(HTTPRequest& request) {
  ...
  std::string uri = request.getURI();
  try {
    request.setConfig(&_server.getLocationConfig(uri));
